...
function selectCurveValue(name, value, girth) {
  ...
  $('#clickItme').text(name)
  $('#clickDate').html(value + '<small>cm</small>')
  var scaleArr = [Math.round(value / 5) * 5 - 5, Math.round(value / 5) * 5, Math.round(value / 5) * 5 + 5]
  console.log(scaleArr);
  for (var k = 1; k <= scaleArr.length; k++) {
    $('#keduBox li').eq(k * 5 - 1).html("<span>"+ scaleArr[k - 1] +"</span>")
  }
}
function initScene() {
  fitCameraToObject();
