<template>
  <div class="box1">
    ...
    <div style="overflow:hidden" class="filter-container">
      ...
      <el-button class="derive" v-if="btnRoleList['7-3-2-1']"  @click="doExport('私教购买修改')" :loading="domnloading"><i class="el-icon-printer"></i> 导出</el-button>
    ...
    <el-table @filter-change="filterChange" @sort-change="sortChange" stripe :key='tableKey' :data="tableList" v-loading="listLoading" element-loading-text="列表正在加载中" border fit highlight-current-row
      ...
      <el-table-column column-key="employee_id" align="center" :label="'操作人'" width="200" prop="scope.row.employee_name"
      ...
      <el-table-column column-key="type" align="center" :label="'操作类型'" width="200" prop="scope.row.type"
      :filters="filterArr"
      ...
      <el-table-column label="操作对象" align="center" class-name="merge-th">
          <el-table-column align="center" :label="'手机号码'" width="200" prop="scope.row.ext">
            <template slot-scope="scope">
              <span>{{scope.row.optobj&&scope.row.optobj.customer?scope.row.optobj.customer.phone:'-'}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" :label="'姓名'" width="200" prop="scope.row.ext">
            <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="'说明'" min-width="230" class-name="small-padding fixed-width" prop="scope.row.remark">
        <template slot-scope="scope">
...
export default {
  ...
  data() {
    let filterArr = actionValue.filter(v => {
      return v.type === 'private-education-purchase'
    })
    filterArr.forEach(v => {
      v.class = 'ctype'
    ...
    return {
      ...
      listQuery: {
        ...
        page: 1,
        per_page: 10,
        logable_type: 'App\\CustomerCourse'
      },
      employeeQuery: {
        page: 1,
        per_page: 1000
  ...
  methods: {
    ...
    opendbatchDetail(e, row) {
      console.log(row)
      if (row.action === 402) { // 私教记录-购买修改-批量删除
