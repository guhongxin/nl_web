      ...
      <div class="content">
       ...
       <div class="content">
        <el-table
          ...
          <el-table-column
            prop="support_gym_names"
            label="支持分店"
            width="150" align="center">
            <template slot-scope="scope">
              <!-- <span v-if="scope.row.support_gym_names">
                <span v-for="(item, key) in scope.row.support_gym_names" :key="key">{{item}}</span>
              </span> -->
              <span>{{tst(scope.row.support_gym_names)[0]}}</span>
                <span style="color:rgb(241,76,48)" v-if="tst(scope.row.support_gym_names)[1]">{{tst(scope.row.support_gym_names)[1]}}</span>
              </template>
          </el-table-column>
          <el-table-column
            prop="sell_gym_name"
            ...
            width="150" align="center">
            <template slot-scope="scope">
              <span>{{tst(scope.row.sell_gym_name)[0]}}</span>
              <span style="color:rgb(241,76,48)" v-if="tst(scope.row.sell_gym_name)[1]">{{tst(scope.row.sell_gym_name)[1]}}</span>
            </template>
          </el-table-column>
           <el-table-column
            prop="real_pay"
...
<script>
// 导入数据错误详情弹窗
import { msCardInpormtErrInfor } from '@/api/mstoreCard'
...
export default {
  ...
  methods: {
    ...
    getVipLog() {
      ...
      msCardInpormtErrInfor(obj).then(res => {
        this.tableData = []
        let data = res.data
        ...
        data.map((item) => {
          ...
          for (let key in item.data) {
            if (item.errors.hasOwnProperty(key)) {
              if (/support_gym_names/.test(key)) {
                let _err = '不支持的分店有:'
                for (let j in item.errors) {
                  if (/support_gym_names/.test(j)) {
                    if (item.data[key][j[j.length - 1]]) {
                      _err += item.data[key][j[j.length - 1]] + '/'
                    }
                  }
                }
                item.data[key] = (item.data[key] ? item.data[key].join('/') : '') + `(${_err.replace(/\/$/, '')})`
              } else {
                item.data[key] = (item.data[key] ? item.data[key] : '') + `(${item.errors[key][0]})`
              }
            } else {
              item.data[key] = item.data[key] ? item.data[key] + '' : ''
            }
          }
    ...
    handleDownload() {
      require.ensure([], () => { // 用 webpack Code Splitting xlsl还是很大的
        ...
        const tHeader = ['序号', '会员卡号', '姓名', '手机号码', '会员卡类型', '会员卡名称', '次卡剩余次数', '办理日期', '开卡日期', '失效日期', '支持分店', '销售分店', '实收金额', '支付方式', '性别', '生日', '身份证号', '备注'] // excel 表格头
        const filterVal = ['excel_col', 'card_no', 'name', 'phone', 'type', 'card_name', 'total_times', 'created_at', 'start_date', 'end_date', 'support_gym_names', 'sell_gym_name', 'real_pay', 'payment_method', 'sex', 'birthday', 'id_card_no', 'remark']
        let obj = {
          data_import_id: this.sid,
          per_page: this.total,
          page: 1
