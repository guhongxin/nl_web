<template>
  <div>
    ...
    <el-table stripe :key='tableKey'
      ...
      <el-table-column align="center" :label="'卡种名称'" prop="card_name" width="200">
        <template slot-scope="scope">
          <span>{{scope.row.card_name}}</span>
        </template>
      </el-table-column>

      <el-table-column width="50px" align="center" :label="'头像'">
        <template slot-scope="scope">
          <span><img @click="common.bigImg($event)" :src="scope.row.customer?(scope.row.customer.head_portrait?scope.row.customer.head_portrait:defaulthead):defaulthead" alt="icon" width="30" height="30" style="display: list-item;border-radius:50%;"></span>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" :label="'办理时间'" width="200" align="center" sortable='custom' prop="created_at_sort">
        <template slot-scope="scope">
        {{common.customFormat(scope.row.bought_at, 'yyyy-MM-dd')}}
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" :label="'开卡时间'" width="200" align="center" sortable='custom' prop="start_date_sort">
        <template slot-scope="scope">
      ...
      <el-table-column  :label="'总次数'" width="120" align="center" prop="total_times">
        <template slot-scope="scope">
         {{scope.row.total_times}}
        </template>
      </el-table-column>
      <el-table-column  :label="'剩余次数'" width="120" align="center" prop="left_times">
        <template slot-scope="scope">
         {{scope.row.total_times - scope.row.use_times}}
        </template>
      </el-table-column>
      <el-table-column :label="'赠次'" width="120" align="center">
        <template slot-scope="scope">
         {{scope.row.give_day?scope.row.give_day : '-'}}
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'销售分店'" width="150" prop="operator_id">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" fixed="right" :label="$t('table.actions')" class-name="small-padding fixed-width" width="150">
        <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" type="primary" @click="handleDetailsBtn(scope.row)">{{$t('table.details')}}</el-button>
    ...
    <vip-card-detail-dialog
      ...
      :isSecondCard="2"
      :widthSize="'950px'"></vip-card-detail-dialog>
...
export default {
  ...
  methods: {
    ...
    searchlist(obj) {
      ...
      delete this.listQuery.template_name_like
