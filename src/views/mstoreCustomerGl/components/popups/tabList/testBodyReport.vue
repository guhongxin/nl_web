<template>
	<div class="testBodyReport">
      ...
      <el-table-column align="center" fixed="right" :label="$t('table.actions')" min-width="150" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="customerDetailsBtn(scope.row,'app')">app查看</el-button>
          <el-button type="primary" size="mini" @click="customerDetailsBtn(scope.row,'pc')">pc查看</el-button>
        </template>
      </el-table-column>
		</el-table>
...
import { msbodyExamList } from '@/api/testReport'
...
export default {
  ...
  methods: {
    ...
    getList(id) {
      ...
      msbodyExamList(this.customer_id, obj).then(res => {
        let data = res.data
        console.log(data)
        this.tableData = data
