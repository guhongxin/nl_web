  ...
  <div class='dialog-content3'>
    <el-row :gutter="20">
      <el-col v-for="(item,key) in contentData3" :key="key" :span="12" style="margin-top:10px;">
        <component v-bind:is="item.vipCardType" :vipCardData="item.VipCardData" @goOpenCard="goOpenCard" @selectAction="selectAction" :isSelect="false"></component>
      </el-col>
    </el-row>
  </div>
</span>
  ...
  methods: {
    ...
    whichPorp(param) {
      ...
      switch (param.selectType) {
        case '时效卡停卡':
          this.$nextTick(() => {
            this.$refs.vipCardStopDialogDoc.openModel(param, false)
          })
          break
        case '次卡扣次':
          this.$nextTick(() => {
            this.$refs.dialogClosedeductVipCardDoc.openModel(param.id, true)
          })
          break
        case '时效卡转卡':
          this.$nextTick(() => {
            this.$refs.dialogtransferCardDoc.openModel(param.id, false)
          })
          break
        case '次卡转卡':
          this.$nextTick(() => {
            this.$refs.dialogtransferCardDoc.openModel(param.id, true)
          })
          break
        case '时效卡续卡':
          this.$nextTick(() => {
            this.$refs.handleContinueDoc.openModel(param.id, false)
          })
          break
        case '次卡续卡':
          this.$nextTick(() => {
            this.$refs.handleContinueDoc.openModel(param.id, true)
          })
          break
        case '时效卡编辑':
          this.$nextTick(() => {
            this.$refs.dialogClosedEditDoc.openModel(param.id, false)
          })
          break
        case '次卡编辑':
          this.$nextTick(() => {
            this.$refs.dialogClosedEditDoc.openModel(param.id, true)
          })
          break
        case '开卡':
          this.$nextTick(() => {
