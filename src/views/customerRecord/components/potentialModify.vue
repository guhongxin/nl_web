<template>
  <div class="box boxer">
    ...
    <div style="overflow:hidden" class="filter-container">
      ...
      <el-button class="derive" v-if="btnRoleList['7-1-2-1']" :loading="domnloading" @click="doExport('潜客修改')"><i class="el-icon-printer"></i> 导出</el-button>
    ...
    <el-table @filter-change="filterChange" @sort-change="sortChange" stripe :key='tableKey' :data="tableList" v-loading="listLoading" element-loading-text="列表正在加载中" border fit highlight-current-row
      ...
      <el-table-column label="操作对象" align="center" class-name="merge-th">
          <el-table-column align="center" :label="'手机号码'" width="200" prop="phone">
            <template slot-scope="scope">
              <span>{{scope.row.optobj?scope.row.optobj.phone:'-'}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" :label="'姓名'" width="200" prop="name">
            <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="'说明'" min-width="300" class-name="small-padding fixed-width" prop="scope.row.remark">
        <template slot-scope="scope">
  ...
  data() {
    ...
    return {
      ...
      listQuery: {
        ...
        page: 1,
        per_page: 10,
        logable_type: 'App\\PotentialCustomer'
      },
      employeeQuery: {
        page: 1,
        per_page: 1000
