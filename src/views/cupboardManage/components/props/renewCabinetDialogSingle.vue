<template>
    <el-dialog
      ...
      <!-- 续费信息确认 -->
        <ul class="ul-box">
            <li>
            <span>当前柜号：</span><span>{{cabinetMsg.locker?cabinetMsg.locker.area:''}}-{{cabinetMsg.no}}</span>
            <span :class="[{ 'red': cabinetMsg.day }, 'delay']">已逾期{{cabinetMsg.day}}天</span>
            </li>
            <li>
            <span>手牌编号：</span><span>{{cabinetMsg.key}}</span>
            </li>
            <li>
            <span>客户信息：</span>
            <img :src="cabinetMsg.locker_rent?cabinetMsg.locker_rent.customer?cabinetMsg.locker_rent.customer.head_portrait? cabinetMsg.locker_rent.customer.head_portrait: defineHead: defineHead:defineHead" alt="" width='60px' height='60px' style="border-radius:50%;vertical-align:top;"/>
            <div class="personal">
                <span>{{cabinetMsg.locker_rent?cabinetMsg.locker_rent.customer?cabinetMsg.locker_rent.customer.name:'':''}}</span><br>
                <span>{{cabinetMsg.locker_rent?cabinetMsg.locker_rent.customer?cabinetMsg.locker_rent.customer.phone:'':''}}</span>
            </div>
            </li>
            <li>
            <span>开始时间：</span>
            <el-date-picker
                v-model="cabinetMsg.locker_rent.started_at"
                type="datetime"
            ...
            <li>
            <span>租用押金：</span>
            <el-input type="number" placeholder="请输入本次租柜的押金" v-model="cabinetMsg.locker_rent.cash_pledge" disabled="disabled"></el-input>
            </li>
            <li>
            <span>实付金额：</span>
      ...
      <div class="cabnit-dialog-footer" style="text-align:center;">
        <span slot="footer" class="dialog-footer">
          <el-button class="return" @click="renewCabinet(cabinetMsg.locker_id, cabinetMsg.id)" :loading="loading" :disabled="submitBug">续费</el-button>
        </span>
      </div>
    </el-dialog>
</template>
...
export default {
  ...
  mounted() {
    let cons = this.cabinetMsg.locker_rent.expired_at
    this.lostTime = {
      disabledDate(time) {
  ...
  created() {
    console.log(this.cabinetMsg.expired_at)
  },
  methods: {
    dialogClosed() {
    ...
    renewCabinet(lockerid, grid) {
      ...
      if (!this.DialogRenew.gmt_end) {
        ...
        return false
      } else if (new Date(this.DialogRenew.gmt_end) - new Date(this.cabinetMsg.locker_rent.expired_at) <= 0) {
        this.common.message('warning', '结束时间必须晚于' + this.cabinetMsg.locker_rent.expired_at)
        return false
      }
