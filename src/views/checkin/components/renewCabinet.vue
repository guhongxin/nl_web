<template>
    <el-dialog
      ...
      <!-- 续费信息确认 -->
        <ul class="ul-box">
            <li>
            <span>当前柜号：</span><span>{{cabinetMsg.locker?cabinetMsg.locker.area:''}}-{{cabinetMsg.locker_grid?cabinetMsg.locker_grid.no:''}}</span>
            <span :class="[{ 'red': cabinetMsg.day }, 'delay']">已逾期{{cabinetMsg.day}}天</span>
            </li>
            <li>
            <span>手牌编号：</span><span>{{cabinetMsg.locker_grid?cabinetMsg.locker_grid.key:''}}</span>
            </li>
            <li>
            <span>会员信息：</span>
            ...
            <!-- <img v-if="!cabinetMsg.customer.head_portrait" src="defineHead" alt="" width='60px' height='60px' style="border-radius:50%;vertical-align:top;"> -->
            <div class="personal">
      ...
      <div class="cabnit-dialog-footer" style="text-align:center;">
        <span slot="footer" class="dialog-footer">
          <el-button class="return" @click="renewCabinet(cabinetMsg.locker.id, cabinetMsg.locker_grid.id)" :loading="loading">续费</el-button>
        </span>
      </div>
    </el-dialog>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.lostTime = {
      disabledDate(time) {
        // return time.getTime() < new Date(cons).getTime() - 1000 * 60 * 60 * 24
  ...
  methods: {
    ...
    renewCabinet(lockerid, grid) {
      ...
      if (Number(this.DialogRenew.real_pay) <= 0) {
        this.common.message('warning', '实付金额必须大于零')
        return false
      }
