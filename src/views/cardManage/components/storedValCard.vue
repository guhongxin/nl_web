<template>
  <div class="poede">
    <el-button class="dapl black" icon="el-icon-sort" v-if="btnRoleList['1-4-3-6']" @click="updloadDialogVisible = true" type="primary">导入</el-button>
    <div class="boxser">
   ...
   <div class="tagbox" v-if="tags.length">
    ...
    <!--   <el-table-column align="center"  :label="'性别'" width="90" prop="scope.row.sex"
      ...
      <el-table-column align="center" :label="'销售人'" width="150" column-key="operator_id" prop="operator_id"
      ...
      <el-table-column :label="'剩余金额'" width="120" align="center" sortable='custom' prop="allDay">
        <template slot-scope="scope">
          {{scope.row.balance}}
        </template>
      </el-table-column>
      <el-table-column :label="'累计充值（金额）'" width="160" align="center" sortable='custom' prop="leftDay">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
          ...
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <!-- <el-dropdown-item @click.native="handleDelete(scope.$index, listData)">确定</el-dropdown-item> -->
              <template v-if="listQuery.status!==7&&listQuery.status!==5">
                <el-dropdown-item  v-if="btnRoleList['1-4-3-1']" @click.native="handleDeduction(scope.$index, listData)" >扣费</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-18']"  @click.native="handleContinue(scope.$index, listData)" >充值</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-2']" @click.native="handleTransferCard(scope.$index, listData)">转卡</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-3']" @click.native="editCard(scope.$index, listData)">编辑</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-4']"  @click.native="handleDelete(scope.$index, listData)" >删除</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-3-20']"  @click.native="cardMember(scope.row)" >成员</el-dropdown-item>
              </template>
              <template v-if="listQuery.status===7">
                <el-dropdown-item  v-if="btnRoleList['1-4-3-12']" @click.native="restoreCardBtn(scope.row)" >恢复</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-17']" @click.native="dgrecycleBin(scope.row)" >删除</el-dropdown-item>
              </template>
              <template v-if="listQuery.status ===5">
                ...
                <el-dropdown-item  v-if="btnRoleList['1-4-3-3']" @click.native="editCard(scope.$index, listData)">编辑</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-4']"  @click.native="handleDelete(scope.$index, listData)" >删除</el-dropdown-item>
                <el-dropdown-item  v-if="btnRoleList['1-4-3-11']"  @click.native="activCardBtn(scope.row)" >开卡</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-3-20']"  @click.native="cardMember(scope.row)" >成员</el-dropdown-item>
              </template>
            </el-dropdown-menu>
          </el-dropdown>
          <!-- <el-button size="mini" v-if="listQuery.status===5&&btnRoleList['1-4-3-11']" @click.native="activCardBtn(scope.row)">开卡</el-button> -->
        </template>
      </el-table-column>
    </el-table>
...
import { findEmployees, reliveCard, oneReliveCard, activeCard, cardbatforcedel,
  dgcardbatforcedel, clearrecyclebin,
  // movetorecyclebin,
  cardInpormtDetails,
  cardInpormtRetryAll, batCardOpen, lockVipCardGl, unlockVipCardGl } from '@/api/vipcardGl'
...
export default {
  ...
  methods: {
    ...
    scaninputover(obj) {
      ...
      this.pageInit()
      if (obj.type !== 'sell_id') {
        // this.tags[0] = { 'name': obj.value, 'type': obj.type }
    ...
    */
    ...
    handleTransferCard(index, data) {
      ...
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogtransferCardDoc.openModel(_data.id, 3)
      })
    },
    handleContinue(index, data) {
      if (data[index].customer) {
        if (data[index].customer.deleted_at) {
          this.common.message('info', '该会员已删除，无法进行充值操作')
          return
        }
      }
      // 弹出续卡弹框
      let _data = data[index]
      this.$refs.handleContinueDoc.openModel(_data.id, 3)
    },
    transferCardCardTj() {
      // 提交转卡窗口
      this.getList()
    ...
    editCard(index, data) {
      ...
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogClosedEditDoc.openModel(_data.id, 3)
      })
    },
    editVipCardTj() {
      // 提交编辑窗口
    ...
    */
    ...
    $fileName(param) {
      ...
      switch (param) {
        case 1:
          ...
          break
        case 2:
          result = '导出即将到期的储值卡.xls'
          break
        case 3:
          result = '导出已失效的储值卡.xls'
          break
        case 4:
          result = '导出停卡中的储值卡.xls'
          break
        case 5:
          result = '导出未卡开的储值卡.xls'
          break
        case 6:
          result = '导出预约停卡的储值卡.xls'
          break
        case 7:
          result = '导出回收站里的储值卡.xls'
          break
        default:
          result = '导出所有储值卡.xls'
          break
    ...
    restoreCardBtn(param) {
      // 回收站-恢复
      this.recoveryId = param.id
    ...
    dgrecycleBin(param) {
      // 单个回收站-删除
      // this.dgrecycleBinDialog = true
      this.dgrecycleId = param.id
      this.$refs.deletionDialogDoc.dialogShow()
      // this.$confirm('是否彻底删除该会员卡?', '提示', {
      //   confirmButtonText: '确定',
      //   cancelButtonText: '取消',
      //   type: 'warning'
      // }).then(() => {
      //   this.dgrecycleBinBtnOk()
      // }).catch(() => {
      //   console.log('取消')
      // })
    },
    ...
    employeeNameList() {
      ...
      employeeList().then(response => {
        ...
        this.coperatorOptions.unshift({ text: '超级管理员', value: '0,超级管理员', class: 'coperator_id' })
      })
    },
    tagclose(tag, index) {
    ...
    handleDeduction(index, data) {
      ...
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogClosedeductVipCardDoc.openModel(_data.id, 3)
      })
    },
    deductVipCardTj() {
      // Vip扣次弹窗提交
