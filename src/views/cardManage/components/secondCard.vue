<template>
  <div class="poede">
    <el-button class="dapl black" icon="el-icon-sort" v-if="btnRoleList['1-4-2-6']" @click="updloadDialogVisible = true" type="primary">导入</el-button>
    <div class="boxser">
    ...
    <div class="find-box3">
      <div style="overflow:hidden" class="filter-container">
        ...
        <!-- <a href="/static/excel/数据导入模板.xlsx" download="数据导入模板" class="down-box" v-if="btnRoleList['1-4-2-10']">下载模板</a> -->
      </div>
    </div>
    ...
    <el-table stripe :key='tableKey'
      ...
      style="width: 100%;" @filter-change="filterChange" @sort-change="sortChange">
      <el-table-column align="center" type="selection" width="55px"></el-table-column>
      <el-table-column align="center" :label="'会员卡号'" prop="scope.row.card_no" width="240px">
      ...
      <el-table-column align="center" :label="'卡种名称'" prop="card_name" width="120px">
       ...
       <!-- <el-table-column align="center" :label="'状态'"  prop="" width="80px">
        <template slot-scope="scope">
          <span>{{'暂无'}}</span>
        </template>
      </el-table-column> -->
      <el-table-column width="110px" align="center" :label="'头像'">
        <template slot-scope="scope">
          <span style="display: inline-block;"><img @click="common.bigImg($event)" :src="scope.row.customer?(scope.row.customer.head_portrait?scope.row.customer.head_portrait:defaulthead):defaulthead" alt="icon" width="30" height="30" style="display: list-item;border-radius:50%;"></span>
          <span v-if="scope.row.members_count>1" class="members-count-box">{{scope.row.members_count}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'姓名'" width="180" prop="customer">
        <template slot-scope="scope">
    ...
    <!--   <el-table-column align="center"  :label="'性别'" width="90" prop="scope.row.sex"
      ...
      <el-table-column class-name="status-col"  label="开卡时间<i onclick='openDate(event)' data=[3,'start_sdate','start_edate'] title='开卡时间' class='el-icon-date'></i>"
      ...
      <el-table-column class-name="status-col" label="失效时间<i onclick='openDate(event)' data=[4,'end_sdate','end_edate'] title='失效时间' class='el-icon-date'></i>"
        ...
        <template slot-scope="scope">
         {{common.customFormat(scope.row.end_date, 'yyyy-MM-dd')}}
        </template>
      </el-table-column>
      <el-table-column v-if="listQuery.status == 3" class-name="status-col" :label="'实际失效时间'" width="200" align="center">
        <template slot-scope="scope">
         {{common.customFormat(scope.row.over_date, 'yyyy-MM-dd')}}
        </template>
      </el-table-column>
      <el-table-column :label="'总次数'" width="120" align="center" sortable='custom' prop="total_times_sort">
        <template slot-scope="scope">
         {{scope.row.total_times}}
        </template>
      </el-table-column>
      <el-table-column :label="'剩余次数'" width="120" align="center" sortable='custom' prop="left_times">
        <template slot-scope="scope">
         {{scope.row.total_times - scope.row.use_times}}
        </template>
      </el-table-column>
      <el-table-column :label="'卡锁定'" width="120" align="center">
        <template slot-scope="scope">
          <el-switch
      ...
      <el-table-column align="center" fixed="right" :label="$t('table.actions')" class-name="small-padding fixed-width" width="150">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleDetailsBtn(scope.$index, listData)">{{$t('table.details')}}</el-button>
          <el-dropdown trigger="click">
            <el-button size="mini" plain>更多</el-button>
            <el-dropdown-menu slot="dropdown">
              <template v-if="listQuery.status!==7&&listQuery.status!==5">
                <el-dropdown-item v-if="btnRoleList['1-4-2-1']" @click.native="handleDeduction(scope.$index, listData)">扣次</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-2']" @click.native="handleTransferCard(scope.$index, listData)">转卡</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-18']" @click.native="handleContinue(scope.$index, listData)">续卡</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-3']" @click.native="editCard(scope.$index, listData)">编辑</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-4']" @click.native="handleDelete(scope.$index, listData)">删除</el-dropdown-item>
                <el-dropdown-item  @click.native="cardMember(scope.row)" >成员</el-dropdown-item>
                <!-- <el-dropdown-item v-if="listQuery.status===5&&btnRoleList['1-4-2-11']" @click.native="activCardBtn(scope.row)">开卡</el-dropdown-item> -->
              </template>
              <template v-if="listQuery.status===7">
                <el-dropdown-item v-if="btnRoleList['1-4-2-12']" @click.native="restoreCardBtn(scope.row)">恢复</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-17']" @click.native="dgrecycleBin(scope.row)">删除</el-dropdown-item>
              </template>
              <template v-if="listQuery.status===5">
                ...
                <el-dropdown-item v-if="btnRoleList['1-4-2-3']" @click.native="editCard(scope.$index, listData)">编辑</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-4']" @click.native="handleDelete(scope.$index, listData)">删除</el-dropdown-item>
                <el-dropdown-item v-if="btnRoleList['1-4-2-11']" @click.native="activCardBtn(scope.row)">开卡</el-dropdown-item>
                <el-dropdown-item  @click.native="cardMember(scope.row)" >成员</el-dropdown-item>
                <!-- <el-dropdown-item v-if="listQuery.status===5&&btnRoleList['1-4-2-11']" @click.native="activCardBtn(scope.row)">开卡</el-dropdown-item> -->
              </template>
            </el-dropdown-menu>
          </el-dropdown>
          <!-- <el-button size="mini" v-if="listQuery.status===5&&btnRoleList['1-4-2-11']" @click.native="activCardBtn(scope.row)">开卡</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    ...
    <vip-card-detail-dialog
      ...
      :widthSize="'950px'"
      :isSecondCard="2"></vip-card-detail-dialog>
    <!-- 编辑信息弹窗  -->
    ...
    <transfer-card-dialog
      ...
      :widthSize="'1300px'"
      :employeesData="sellList"></transfer-card-dialog>
    <!-- 扣次 -->
...
import DeductVipCardDialog from './DeductVipCardDialog'
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.updateFormData = Object.assign({}, row)
    ...
    handleContinue(index, data) {
      ...
      // 弹出续卡弹框
      let _data = data[index]
      this.$refs.handleContinueDoc.openModel(_data.id, 2)
    },

    handleDeduction(index, data) {
      if (data[index].customer) {
        if (data[index].customer.deleted_at) {
          this.common.message('info', '该会员已删除，无法进行扣次操作')
          return
        }
      }
      // 扣次
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogClosedeductVipCardDoc.openModel(_data.id, 2)
      })
    },
    deductVipCardTj() {
      // Vip扣次弹窗提交
    ...
    handleTransferCard(index, data) {
      ...
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogtransferCardDoc.openModel(_data.id, 2)
      })
    },
    transferCardCardTj() {
      // 转卡提交
    ...
    editCard(index, data) {
      ...
      let _data = data[index]
      this.$nextTick(() => {
        this.$refs.dialogClosedEditDoc.openModel(_data.id, 2)
      })
    },
    editVipCardTj() {
      // 编辑提交
    ...
    $fileName(param) {
      ...
      switch (param) {
        case 1:
          ...
          break
        case 2:
          result = '导出即将到期的次卡.xlsx'
          break
        case 3:
          result = '导出已失效的次卡.xlsx'
          break
        case 4:
          result = '导出停卡中的次卡.xlsx'
          break
        case 5:
          result = '导出未开卡的次卡.xlsx'
          break
        case 6:
          result = '导出预约停卡的次卡.xlsx'
          break
        case 7:
          result = '导出回收站里的次卡.xlsx'
          break
        default:
          result = '导出所有次卡.xlsx'
          break
