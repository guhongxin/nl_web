<template>
  <div>
    ...
    <el-table @filter-change="filterChange" stripe 
      ...
      <el-table-column align="center" :label="'商品名称'" prop="customerName" width="150px">
        <template slot-scope="scope">
          <div class="mytd" v-for="(item, index) in scope.row.goods_order_details" :key="index">{{item.name}}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'单价'" prop="price_sort" width="150px">
        <template slot-scope="scope">
          <div class="mytd" v-for="(item, index) in scope.row.goods_order_details" :key="index">{{item.price}}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'购物数量'" prop="num_sort" width="150px">
        <template slot-scope="scope">
          <div class="mytd" v-for="(item, index) in scope.row.goods_order_details" :key="index">{{item.quantity}}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'客户姓名'" prop="customerName" width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="'操作'" min-width="120" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleCheck(scope.row)" v-if="btnRoleList['6-2-4-2']" v-show="scope.row.source!==1">财务核对</el-button>
        </template>
      </el-table-column>
    </el-table>
