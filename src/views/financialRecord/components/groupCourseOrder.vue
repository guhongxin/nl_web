  ...
  <div>
    ...
    <el-table @filter-change="filterChange" stripe 
      ...
      <el-table-column column-key="sex" align="center" :label="'课程名称'" prop="cardno" width="150">
        <template slot-scope="scope">
          <span>{{scope.row.detail?scope.row.detail.name:''}}</span>
        </template>
      </el-table-column>
      ...
      :filters="coachOption" filter-placement="bottom-end" :filter-multiple="false">
        <template slot-scope="scope">
          <span>{{scope.row.detail?scope.row.detail.coach?scope.row.detail.coach.name:'':''}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="上课时间<i class='el-icon-date'></i>" prop="course_time_sort" width="200">
        <template slot-scope="scope">
          <span>{{scope.row.detail?scope.row.detail.schedule?scope.row.detail.schedule.start_at:'':''}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'客户姓名'" prop="customerName">
        <template slot-scope="scope">
          <span v-if="!getDeleteCustomer(scope.row.detail?scope.row.detail.schedule?scope.row.detail.schedule.delete_at:'':'')">{{scope.row.customer?scope.row.customer.name:''}}</span>
          <el-tooltip class="item" effect="dark" v-if="getDeleteCustomer(scope.row.detail?scope.row.detail.schedule?scope.row.detail.schedule.delete_at:'':'')"  :content="`客户已删除，删除时间: ${scope.row.customer.deleted_at}`" placement="bottom">
            <el-button type="info" size="mini">{{scope.row.customer?scope.row.customer.name:'-'}}</el-button>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="'手机号码'" prop="phone_sort" width="130">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="'操作'" min-width="120" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleCheck(scope.row)" v-if="btnRoleList['6-2-3-2']" v-show="scope.row.source!==1">财务核对</el-button>
        </template>
      </el-table-column>
    </el-table>
