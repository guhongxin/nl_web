<template>
      ...
      <el-row>
        <el-col :span="12" style="border-right:1px solid #D8D8D8">
          <el-row>
            <el-col :span="12">
              <h3>总消耗</h3>
              <p class="total-p">{{getNumberInt(sportsTotal.total_calorie?sportsTotal.total_calorie/1000:'0')}}<span>kcal</span></p>
            </el-col>
            <el-col :span="12">
              <h3>总运动</h3>
              <p class="total-p">{{getNumberInt(sportsTotal.total_calorie_sec?(sportsTotal.total_calorie_sec/60).toFixed(1):'0')}}<span>min</span></p>
            </el-col>
          </el-row>
        </el-col>
        <el-col :span="12">
            ...
            <el-col :span="8">
              <div class="statistical-box">
                ...
                <p class="canvas-data" v-if="this.sportMedal.data_force">  
                  <span class="sport-data-box" 
                    :title="getNumberInt(this.sportMedal.data_force.amount)+ '/'+ this.sportMedal.data_force.max_amount + this.sportMedal.data_force.unit">
                    {{getNumberInt(this.sportMedal.data_force.amount)}}/{{this.sportMedal.data_force.max_amount}}
                    <span>{{this.sportMedal.data_force.unit}}</span>
                  </span>
                </p>
            ...
            <el-col :span="8">
              <div class="statistical-box">
                ...
                <p class="canvas-data" v-if="this.sportMedal.data_aerobic">
                  <span class="sport-data-box" 
                    :title="getNumberInt(this.sportMedal.data_aerobic.amount)+ '/' + this.sportMedal.data_aerobic.max_amount+this.sportMedal.data_aerobic.unit">
                    {{getNumberInt(this.sportMedal.data_aerobic.amount)}}/{{this.sportMedal.data_aerobic.max_amount}}
                    <span>{{this.sportMedal.data_aerobic.unit}}</span>
                  </span>
                </p>
                <p class="canvas-data" v-else>-/-<span>-</span></p>
              </div>
            ...
            <el-col :span="8">
              <div class="statistical-box">
                ...
                <p class="canvas-data" v-if="this.sportMedal.data_walk">
                  <span class="sport-data-box" 
                    :title="getNumberInt(this.sportMedal.data_walk.amount)+ '/' + this.sportMedal.data_walk.max_amount+this.sportMedal.data_walk.unit">
                    {{getNumberInt(this.sportMedal.data_walk.amount)}}/{{this.sportMedal.data_walk.max_amount}}
                    <span>{{this.sportMedal.data_walk.unit}}</span>
                  </span>
                </p>
                 <p class="canvas-data" v-else>-/-<span>-</span></p>
              </div>
      ...
      style="width: 100%;">
        ...
        <template slot-scope="scope">
          <span>{{scope.row.sport_type_name}}</span>
			...
			>
        <template slot-scope="scope">
          <span v-if="scope.row.sport_content_type === 1">{{getNumberFix(getRunContent(scope.row)/100000)}}<span>{{getunit(scope.row.sport_content_type)}}</span></span>
          <span v-else-if="scope.row.sport_content_type === 2">{{getNumberFix(getStrength_content(scope.row))}}<span>{{getunit(scope.row.sport_content_type)}}</span></span>
          <span v-else>{{getNumberInt(getStepContent(scope.row))}}<span>{{getunit(scope.row.sport_content_type)}}</span></span>
        </template>
			</el-table-column>
			<el-table-column
				label="消耗卡路里"
			...
			>
        <template slot-scope="scope">
          <span>{{parseInt((scope.row.sport_aerobic_calorie + scope.row.sport_anaerobic_calorie + scope.row.sport_stamina_calorie)/1000)}} kcal</span>
        </template>
			</el-table-column>
		</el-table>
    <div class="pagination-container"  style="text-align:right;">
        ...
        <el-button-group>
          <el-button type="primary" icon="el-icon-arrow-left" :disabled="prev_pageBtnDisabled" @click="prevBtn">上一页</el-button>
          <el-button type="primary" :disabled="next_pageBtnDisabled" @click="nextBtn">下一页<i class="el-icon-arrow-right el-icon--right"></i></el-button>
        </el-button-group>
      </div>
	</div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      prev_pageBtnDisabled: true, // 上一页是否禁用
      next_pageBtnDisabled: false // 下一页是否禁用
    }
  },
  created() {},
  mounted() {},
  watch: {
    firstId() {
      this.prev_pageBtnDisabled = !(this.firstId.length > 0)
    },
    tableData() {
      this.next_pageBtnDisabled = this.tableData.length < this.per_page
    }
  },
  methods: {
    getNumberInt(value) {
    ...
    getNumberFix(value) {
      return this.common.float2(value)
    },
    ...
    getStrength_content(param) {
      // 获取器械的
      if (!param.SportContent || !param.SportContent.StrengthContent || !param.SportContent.StrengthContent.total_amount) {
        return 0
      }
      return param.SportContent.StrengthContent.total_amount
    },
    prevBtn() {
      // 上一页按钮
      // 如果firstId为空数组，说明已经到了第一页，禁用上一页按钮
      let _firstId = this.firstId.pop()
      if (_firstId && _firstId.length !== 0) {
        this.findId = _firstId[0]
        this.getapp_sports_detail(this.customer_id)
      } else {
    ...
    nextBtn() {
      ...
      this.next_pageBtnDisabled = false
      if (this.firstId.indexOf(this.tableData[0].sport_date) === -1) {
        if (this.firstId.length === 0) {
          // 第一页sport_date是从[0, this.tableData[this.tableData.length - 1].sport_date]
          this.firstId.push([0, this.tableData[this.tableData.length - 1].sport_date])
