<template>
  <div class="box1">
    ...
    <div style="overflow:hidden" class="filter-container">
      ...
      <el-button class="derive" v-if="btnRoleList['7-5-1-1']" @click="doExport('团课课程修改')" :loading="domnloading"><i class="el-icon-printer"></i> 导出</el-button>
    ...
    <el-table @filter-change="filterChange" @sort-change="sortChange" stripe :key='tableKey' :data="tableList" v-loading="listLoading" element-loading-text="列表正在加载中" border fit highlight-current-row
      ...
      <el-table-column column-key="employee_id" align="center" :label="'操作人'" prop="employee_name"
      ...
      <el-table-column label="操作对象" align="center" class-name="merge-th">
          <el-table-column align="center" :label="'课程名称'" prop="group_course_id" column-key="group_course_id"
          :filters="courseOptions" filter-placement="bottom-end" :filter-multiple="false">
            <template slot-scope="scope">
              <span>{{scope.row.optobj?scope.row.optobj.name:'-'}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" :label="'课程类型'" column-key="group_course_type" prop="group_course_type"
          :filters="fitTypeOptions" filter-placement="bottom-end" :filter-multiple="false">
            <template slot-scope="scope">
              <span>{{scope.row.optobj?getfit_type(scope.row.optobj.effect):'-'}}</span>
            </template>
          </el-table-column>
      </el-table-column>
...
import { groupCourseNameList } from '@/api/course'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tags: [],
      fitTypeOptions: [
        { text: '减脂', value: 'J' + ',' + '减脂', class: 'cgroup_course_type' },
        { text: '增肌', value: 'Z' + ',' + '增肌', class: 'cgroup_course_type' },
        { text: '塑形', value: 'S' + ',' + '塑形', class: 'cgroup_course_type' }
      ],
      courseOptions: [],
      select: '',
      tableKey: 0,
      ...
      listQuery: {
        ...
        page: 1,
        per_page: 10,
        logable_type: 'App\\UnfreeGroupCourse'
      },
      employeeQuery: {
        page: 1,
        per_page: 1000
  ...
  methods: {
      ...
      groupCourseNameList().then(response => {
        ...
        if (data.length > 0) {
          for (let i = 0; i < data.length; i++) {
            this.courseOptions.push({ text: response.data[i].name, value: response.data[i].id + ',' + response.data[i].name, class: 'cgroup_course_id' })
          }
        }
      })
    },
    ...
    computedEditValue(value) {
      if (value && value.ext && value.ext.dirty) {
        ...
        delete selfactionArr['hall_id']
        for (var key in selfactionArr) {
          let oldV = value.ext.old[key] ? value.ext.old[key] : ''
          let newV = value.ext.dirty[key] ? value.ext.dirty[key] : ''
          if (key === 'course_date') {
            oldV = oldV.slice(0, 11)
            newV = newV.slice(0, 11)
          } else if (key === 'effect') {
            oldV = this.getfit_type(oldV)
            newV = this.getfit_type(newV)
          } else if (key === 'is_app_sale') {
            oldV = oldV ? '是' : '否'
            newV = newV ? '是' : '否'
          }
          txt += (getype(key) + '：' + oldV + '→' + newV + ' \n ')
        }
        return txt
      }
      return '--'
      ...
      logList(obj).then(response => {
        ...
        data.forEach((item, index) => {
          let middleArry = [
            ...
            item.optobj ? this.getfit_type(item.optobj.effect) : '',
            this.computedRemark(item)
          ]
