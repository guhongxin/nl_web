<template>
    <div class="vue-image-crop-upload" v-show="show">
        <div class="vicp-wrap">
            ...
            <div class="vicp-step1" v-if="step == 1">
    ...
    */
    ...
    setSourceImg(file) {
      ...
      fr.onload = function(e) {
        ...
        if (that.nature) {
          that.goUpload(that.sourceImgUrl, file.type);
        } else {
          that.startCrop();
        }
      };
    ...
    // 获取
    getUploadURL(url, type, md5) {
      return axios.post(url, {
        api_token: getToken() || this.token,
        type: type
        // content_md5: md5
      });
    },
    ...
    upload() {
      ...
      that.goUpload(createImgUrl, mime);
      /* 
      request({
        url,
        method: 'post',
    ...
    goUpload(sourceImgUrl, type) {
      let that = this;
      let imgInfo = data2blob(sourceImgUrl, type);
      let fileReader = new FileReader();
      fileReader.readAsArrayBuffer(imgInfo);
      fileReader.onload = function(evt) {
        if (evt.target.readyState == FileReader.DONE) {
          //二进制数据结果
          let imgFlag = evt.target.result;
          let spark = new SparkMD5.ArrayBuffer();
          spark.append(imgFlag);
          let fileMD5 = spark.end();
          let _iStoken = getSession('Phone-Token')
          let _url = that.isToken ? that.url + `?token=${_iStoken}` : that.url
          // that.getUploadURL(that.url, type, fileMD5).then(res => {
          that.getUploadURL(_url, type, fileMD5).then(res => {
            that.reset();
            that.loading = 1;
            that.setStep(3);
            that.$emit(
            ...
            xhr.upload.onprogress = function(evt) {
              if (evt.lengthComputable) {
                that.progress = Math.ceil(100 * Math.round(evt.loaded) / evt.total);
                console.log("正在提交." + that.progress.toString() + "%"); //在控制台打印上传进度
              }
            };
            xhr.onload = function(e) {
              console.log(e);
              that.loading = 2;
              that.$emit("crop-upload-success", {
                name: res.data.filename,
                url:
                  "https://" + res.data.bucket + "." + res.data.endpoint + "/"
              });
            };
            xhr.onerror = function(e) {
              ...
              that.$emit("crop-upload-fail", that.err, that.field, that.ki);
            };
            xhr.send(imgFlag);
          })
          .catch(err => {
            console.log(err.response.data.message);
            let statusCode = err.response.data.status_code
            let errorCode = err.response.data.error_code
            if (statusCode === 403) {
              if (errorCode === 30003) {
                that.common.message('error','验证码已经过期，请重新获取验证码')
                that.step = 1
                that.off()
                that.$emit('uploadErro')
              } else {
                that.common.message('无权访问此页面！')
              }
            }
          });
        }
